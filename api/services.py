...
def get_routes(route_name: str = '') -> list:
    ...
    try:
        ...
        raise TransmilenioAPIError('Tiempo de espera agotado al conectar con Transmilenio') from None
    except requests.exceptions.ReadTimeout:
        logger.error(f'Read timeout when fetching routes for {route_name}')
        raise TransmilenioAPIError('Transmilenio tardó demasiado en responder') from None
    except requests.exceptions.ProxyError:
        logger.error(f'Proxy error when fetching routes for {route_name}')
        raise TransmilenioAPIError('Error de conexión con Transmilenio') from None
    except requests.exceptions.ConnectionError:
        logger.error(f'Connection error when fetching routes for {route_name}')
        raise TransmilenioAPIError('No se pudo conectar con Transmilenio') from None
    except requests.exceptions.HTTPError as e:
        logger.error(f'HTTP error {e.response.status_code} when fetching routes for {route_name}: {e}')
        raise TransmilenioAPIError(f'Transmilenio respondió con error ({e.response.status_code})') from e
    except requests.exceptions.RequestException as e:
        logger.error(f'Unexpected error when fetching routes for {route_name}: {e}')
        raise TransmilenioAPIError('Error inesperado al consultar Transmilenio') from e
    ...
    try:
        ...
        raise TransmilenioAPIError('Respuesta inválida de Transmilenio') from e
    ...
    try:
        ...
        raise TransmilenioAPIError('Datos incompletos en la respuesta de Transmilenio') from e
    except TypeError:
        logger.error(f'Invalid data format in route response for {route_name}')
        raise TransmilenioAPIError('Formato de datos inesperado de Transmilenio') from None
...
def find_stations_for_route(route_id: str = '') -> list:
    ...
    headers = {
        'User-Agent': ('MetroBus/2.50 (com.nexura.transmilenio; build:345; iOS 26.2.0) Alamofire/4.9.1'),
    }
    ...
    try:
        ...
        raise TransmilenioAPIError('Error inesperado al procesar datos de estaciones') from e
